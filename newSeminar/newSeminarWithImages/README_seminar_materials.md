...
- `slides/<slug>_slides.tex` — **refined** Beamer slide deck: expanded content, a TikZ architecture diagram, an example-walkthrough slide, and embedded speaker notes (compile with pdflatex, run twice for the table of contents)
...
Each deck needs `tikz` and `booktabs` (both ship with any standard TeX distribution such as TeX Live or MiKTeX — no extra install).

## Speaker notes

Every deck has speaker notes embedded via `\note{...}`. They are **hidden by default**.
To generate a version WITH note pages, open the deck and uncomment this line near the top:

```latex
% \setbeameroption{show notes}
```

Tip: keep two copies (one with notes shown for yourself, one without for projecting), or use
`\setbeameroption{show notes on second screen=right}` for a presenter view.

## Architecture diagrams

The **System Architecture** slide in each deck is a real TikZ diagram (color-coded: blue = input/user,
green = agent/process, red = attacker/malicious, violet = defense/guard, yellow = decision, orange = output,
grey = data/tools). To tweak one, edit the `\node[...] (id) at (x,y) {label};` and `\draw[flow] (a) -- (b);`
lines on that slide — positions are simple `(x,y)` coordinates.
